...
> 4. **Published pieces** live at `pages.readystack.ai/{handle}`, fetched by handle. Share
>    attribution rides in the query string as `?s={shareCode}`, so the path stays clean for
>    SEO and no per-sharer routing is needed.
